...
> 整理自 [菜鸟教程 · Linux 常用命令集合](https://www.runoob.com/w3cnote/linux-common-command.html)，按使用场景分类，供日常速查。
>
> Linux 基础原理（文件系统、inode、进程等）见 [Linux 基础](./README.md)；进程管理实操（ps/kill/后台运行）、文件传输（scp/wget）见 [Linux 常用操作](./linux.md)。
...
iconv -f fromEncoding -t toEncoding inputFile > outputFile
                        # 假设 inputFile 按 fromEncoding 编码，转换成 toEncoding 后输出新文件
```
...
mount /dev/sda1 /mnt/usbdisk  # 挂载一个 usb 优盘或闪存设备
...
chmod ugo+rwx directory1      # 设置目录的所有人(u)、群组(g)以及其他人(o)
                              # 以读（r）、写(w)和执行(x)的权限
...
# 使用管道、STDIN 和 STDOUT 进行文本操作的通用语法
...
sort file1 file2 | uniq -d       # 取出两个文件的交集（只留下同时存在于两个文件中的行）
...
dd if=/dev/hda of=/dev/fd0 bs=512 count=1 # 将 MBR (Master Boot Record) 内容复制到软盘
...
route add -net 192.168.0.0 netmask 255.255.0.0 gw 192.168.1.1
                    # 配置到达网络 '192.168.0.0/16' 的静态路由
route del 0/0 gw IP_gateway              # 删除静态路由
echo "1" > /proc/sys/net/ipv4/ip_forward # 激活 ip 路由

hostname            # 显示系统的主机名
host www.example.com   # 查找主机名以将名称解析成 ip 地址（反之亦然）(1)
nslookup www.example.com # 查找主机名以将名称解析成 ip 地址（反之亦然）(2)

ip link show        # 显示所有接口的链路状态
mii-tool eth0       # 显示 'eth0' 的链路状态
ethtool eth0        # 显示网卡 'eth0' 的统计数据
netstat -tup        # 显示所有活动的网络连接及其 PID
netstat -tupl       # 显示系统中所有监听的网络服务及其 PID
...
> 注：`ifconfig`/`netstat`/`route` 已逐渐被 `ip`、`ss` 等新工具取代（iproute2 套件），新系统建议优先使用 `ip addr`、`ss -tupl` 等。

## Microsoft Windows 网络（SAMBA）

```bash
nbtscan ip_addr     # netbios 名称解析
nmblookup -A ip_addr # netbios 名称解析
smbclient -L ip_addr/hostname # 显示一台 windows 主机的远程共享
smbget -Rr smb://ip_addr/share # 像 wget 一样，可以通过 smb 从主机 windows 下载文件
mount -t smbfs -o username=user,password=pass //WinClient/share /mnt/share
                    # 挂载一个 windows 网络共享
```
