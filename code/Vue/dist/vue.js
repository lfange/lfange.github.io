(function (global, factory) {
  ...
  //函数劫持
  // 1 获取到数组原型上的方法（原来的方法）
  var oldArrayProtoMethods = Array.prototype; //2继承一下
  ...
  var Dep = /*#__PURE__*/function () {
    ...
    _createClass(Dep, [{
      ...
      value: function depend() {
        ...
        Dep.targer.addDep(this); // this.subs.push(Dep.targer) // id：1 记住他的dep
      }
    }, {
      key: "addSub",
  ...
  function defineReactive(data, key, value) {
    ...
    Object.defineProperty(data, key, {
      get: function get() {
        ...
        if (Dep.targer) {
          //让这个属性记住这个watcher
          dep.depend(); //3当我们对arr取值的时候 我们就让数组的dep记住这个watcher
  ...
  function nextTick(cb) {
    ...
    //  console.log(cb)
    //列队 [cb1,cb2]
    callback.push(cb); //Promise.then()  vue3
  ...
  function stateMixin(vm) {
    ...
    vm.prototype.$nextTick = function (cb) {
      ...
      //上面格式化处理
      console.log(exprOrfn, handler, options); //实现watch 方法 就是new  watcher //渲染走 渲染watcher $watch 走 watcher  user false
      //
  ...
  function start(tagName, attrs) {
    ...
    var element = createASTELement(tagName, attrs); //注意：是不是根元素
  ...
  function end(tagName) {
    ...
    currentParent = stack[stack.length - 1]; // debugger
  ...
  function genProps(attrs) {
    ...
    for (var i = 0; i < attrs.length; i++) {
      var attr = attrs[i]; //注意;   style："color:red;font-size: 20px
  ...
  function generate(el) {
    // console.log(el)
    var children = genChildren(el); //方法 拼接字符串  源码也是这样操作 [{}]    ${el.attrs.length?`{style:{color:red}}`:'undefined'}
  ...
  function compileToFunction(template) {
    //编译模板
    var ast = parseHTML(template); //2通过这个棵树重新的生成代码  render
    ...
    return render; //后面渲染我们的节点 ，再放到页面上去
  } // //with   添加一个作用域
  // let obj ={a:1,b:2}
  // with(obj ){
  //       console.log(a,b)
  ...
  var Watcher = /*#__PURE__*/function () {
    ...
    function Watcher(vm, exprOrfn, cb, options) {
      ...
      this.cb = cb;
      this.options = options; // 2. 每一组件只有一个watcher 他是为标识

      this.id = id++; // 3.判断表达式是不是一个函数
    ...
    _createClass(Watcher, [{
      ...
      value: function addDep(dep) {
        ...
        var id = dep.id; //  console.log(dep.id)

        if (!this.depsId.has(id)) {
          this.deps.push(dep);
          this.depsId.add(id); //同时将watcher 放到 dep中
          // console.log(666)
  ...
  function mountComponent(vm, el) {
    ...
    //源码方式
    callHook(vm, "beforeMount"); //挂载  重新封装一下 变成一个类，这个类他是响应式变的，数据改变了我们就调用这个方法
  ...
  function mergeOptions(parent, child) {
    var options = {}; // console.log(parent) {created:[a]}
    //遍历父亲：可能是父亲有，儿子没有
  ...
  function initMixin(Vue) {
    ...
    Vue.prototype._init = function (options) {
      ...
      callHook(vm, "created"); //vue核心特点  响应式数据原理
      //vue 是一个什么样的框架 mvvm
    ...
    Vue.prototype.$mount = function (el) {
      ...
      vm.$el = el; //现在我们就希望渲染页面
      //1: 如果有render  渲染render

      if (!options.render) {
        // 没有
        var template = options.template; //1.1 需要判断有没有template
  ...
  function renderMixin(Vue) {
    ...
    Vue.prototype._render = function () {
      ...
      //获取render 函数
      var vm = this;
      var render = vm.$options.render; //执行render 函数

      var vnode = render.call(this); // console.log(vnode) //展示生成的vnode,就描述的dom结构 好处
  ...
  function initGlobalApi(Vue) {
    ...
    Vue.mixin = function (mixin) {
      ...
      this.options = mergeOptions(this.options, mixin); //  console.log( Vue.options,99999)
    };
